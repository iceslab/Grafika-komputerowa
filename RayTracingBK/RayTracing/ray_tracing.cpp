...
Point3D   startingPoint = { 2.0, 2.0, 0.0};						//punkt, z ktorego wychodzi promien
...
void idle()
{
	spheres[0].rotateX(0.1);
	spheres[1].rotateX(0.1);

	// cout << flow(spheres[0].position, <<", " <<) << endl;

	for( unsigned i = 2; i < spheres.size(); ++i)
