...
void MyInit(void)
{
	...
	pBytes = LoadTGAImage("D2_t.tga", &ImWidth, &ImHeight, &ImComponents, &ImFormat);
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case ' ':
		...
		break;
	case '1':
		drawSide[0] = !drawSide[0];
		break;
	case '2':
		drawSide[1] = !drawSide[1];
		break;
	case '3':
		drawSide[2] = !drawSide[2];
		break;
	case '4':
		drawSide[3] = !drawSide[3];
		break;
	case '5':
		drawSide[4] = !drawSide[4];
		break;
	case 27:
		exit(0);
	default:
