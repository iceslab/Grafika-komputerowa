...
void Reflect2(Point3D p, Point3D intersection, Float3 normalVec, Float3 reflectedVec) {
	float	n_dot_i;
	float   invert[3] = { -p[0], -p[1], -p[2] };

	Normalization(invert);

	n_dot_i = Scalar(invert, normalVec);
	reflectedVec[0] = 2 * (n_dot_i)*normalVec[0] - invert[0];
	reflectedVec[1] = 2 * (n_dot_i)*normalVec[1] - invert[1];
	reflectedVec[2] = 2 * (n_dot_i)*normalVec[2] - invert[2];

	Normalization(reflectedVec);
}

//Funkcja oblicza wektor normalny w punkcie
void Normal2(Float3 normalVec, Point3D intesection, int sphereHit)
{
	normalVec[0] = intesection[0] - spheres[sphereHit].position[0];
	normalVec[1] = intesection[1] - spheres[sphereHit].position[1];
	normalVec[2] = intesection[2] - spheres[sphereHit].position[2];

	Normalization(normalVec);
}

int Intersect2(Point3D p, Float3 d, Point3D q) {
	float r, a, b, c, delta;
	float distance = numeric_limits<float>::max();
	int sphereHit = -1;

	for (unsigned i = 0; i < spheres.size(); i++) {
		a = sq(d[0]) + sq(d[1]) + sq(d[2]);
		b = 2 * (d[0] * (p[0] - spheres[i].position[0])
			+ d[1] * (p[1] - spheres[i].position[1])
			+ d[2] * (p[2] - spheres[i].position[2]));
		c = sq(p[0]) + sq(p[1]) + sq(p[2])
			- 2 * (spheres[i].position[0] * p[0]
			+ spheres[i].position[1] * p[1]
			+ spheres[i].position[2] * p[2])
...
void Phong2(Point3D intersection, Float3 normalVec, Float3 d, Color3 local, int sphereHit)
{
	...
	{
		lightVec[0] = lights[i].position[0] - intersection[0]; //wektor wskazujacy kierunek zrodla
		lightVec[1] = lights[i].position[1] - intersection[1];
		lightVec[2] = lights[i].position[2] - intersection[2];

		Normalization(lightVec);    //normalizacja wektora kierunku swiecenia zrodla           

		n_dot_l = Scalar(lightVec, normalVec);

		reflectionVec[0] = 2 * (n_dot_l)*normalVec[0] - lightVec[0];
		reflectionVec[1] = 2 * (n_dot_l)*normalVec[1] - lightVec[1];
		reflectionVec[2] = 2 * (n_dot_l)*normalVec[2] - lightVec[2];
		...
		{                   //oswietlenie wyliczane jest ze wzorow dla modelu Phonga
			float x = sqrt(sq(lights[i].position[0] - intersection[0]) + sq(lights[i].position[1] - intersection[1]) + sq(lights[i].position[2] - intersection[2]));
			local[0] += (1.0 / (1.0 + 0.01*x + 0.001*x*x))*(spheres[sphereHit].diffuse[0] * lights[i].diffuse[0] * n_dot_l + spheres[sphereHit].specular[0] * lights[i].specular[0] * pow(double(v_dot_r), (double)spheres[sphereHit].shine)) + spheres[sphereHit].ambient[0] * lights[i].ambient[0];
			local[1] += (1.0 / (1.0 + 0.01*x + 0.001*x*x))*(spheres[sphereHit].diffuse[1] * lights[i].diffuse[1] * n_dot_l + spheres[sphereHit].specular[1] * lights[i].specular[1] * pow(double(v_dot_r), (double)spheres[sphereHit].shine)) + spheres[sphereHit].ambient[1] * lights[i].ambient[1];
			local[2] += (1.0 / (1.0 + 0.01*x + 0.001*x*x))*(spheres[sphereHit].diffuse[2] * lights[i].diffuse[2] * n_dot_l + spheres[sphereHit].specular[2] * lights[i].specular[2] * pow(double(v_dot_r), (double)spheres[sphereHit].shine)) + spheres[sphereHit].ambient[2] * lights[i].ambient[2];
		}
		else
		{
		...
		local[0] += spheres[sphereHit].ambient[0] * global_ambient[0];
		local[1] += spheres[sphereHit].ambient[1] * global_ambient[1];
		local[2] += spheres[sphereHit].ambient[2] * global_ambient[2];
	}
...
void Trace2(Point3D p, Float3 d, int step, Color3 color)
{
	if (step > maxSteps)
	{
		//color[0] = backcolor[0];
	...
	{
		Color3 local, reflected;
		Point3D intersection;
		Float3 normalVec, reflectedVec;

		auto sphereHit = Intersect2(p, d, intersection);
		...
		{
			Normal2(normalVec, intersection, sphereHit);
			Reflect2(p, intersection, normalVec, reflectedVec);
			Phong2(intersection, normalVec, d, local, sphereHit);
			Trace2(intersection, reflectedVec, step + 1, reflected);

			// Tak docelowo ma byæ
			color[0] = local[0] + reflected[0];
			color[1] = local[1] + reflected[1];
			color[2] = local[2] + reflected[2];

			//color[0] = local[0] + reflected[0] > 1.0 ? 1.0 : local[0] + reflected[0];
			//color[1] = local[1] + reflected[1] > 1.0 ? 1.0 : local[1] + reflected[1];
			//color[2] = local[2] + reflected[2] > 1.0 ? 1.0 : local[2] + reflected[2];

			//color[0] = local[0];// + reflected[0];
			//color[1] = local[1];// + reflected[1];
			//color[2] = local[2];// + reflected[2];

			//color[0] = reflected[0];// + local[0];
			//color[1] = reflected[1];// + local[1];
			//color[2] = reflected[2];// + local[2];
		}
	}
